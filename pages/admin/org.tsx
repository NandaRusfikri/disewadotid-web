...
export async function getServerSideProps() {
    ...
    try {
        const ListUserResponse = await axios.post(process.env.apidomain + '/api/v1/organization/list', {});
        const ListOrg = ListUserResponse.data.data;



        return {props: { ListOrg}};
    } catch (error) {
        console.error(error);
        return {props: { ListOrg: []}};
    }
...
const Org: React.FC<OrgProps> = ({ ListOrg, error}) => {
    if (error) {
                                ...
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    ...
                                    {ListOrg.map((organization) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">{organization.province}</td>
                                            <td className="px-6 py-4">{organization.city}</td>
                                            <td className="px-6 py-4">{organization.phone}</td>
                                            <td className="px-6 py-4">{organization.maps_url}</td>
